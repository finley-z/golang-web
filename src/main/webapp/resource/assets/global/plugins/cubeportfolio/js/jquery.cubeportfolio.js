...
(function($, window, document, undefined) {
    ...
    function CubePortfolio(obj, options, callback) {
        ...
        // default interceptor for plugin
    ...
    $.extend(CubePortfolio.prototype, {
        ...
        computeFilter: function(expression) {
            ...
            // interceptor call layout
            t.filterLayout();
        },


        /**
         *  Default interceptor layout if nothing overrides
         */
        ...
        /**
         *  Trigger when a interceptor is finished
         */
        ...
        computeBlocks: function(expression) {
            ...
            // blocks that are visible before applying the interceptor
            t.blocksOnInitial = t.blocksOn;

            // blocks visible after applying the interceptor
            t.blocksOn = t.blocks.filter(expression);

            // blocks off after applying the interceptor
...
jQuery.fn.cubeportfolio.options = {
     ...
     *  Values: strings that represent the interceptor name(ex: *, .logo, .controller-design, .design)
     */
    defaultFilter: '*',
    ...
    /**
     *  Defines which animation to use for items that will be shown or hidden after a interceptor has been activated.
     *  Option available only for `layoutMode: 'grid'`
    ...
    CubePortfolio.public = {
        ...
        filter: function(param, callback) {
            ...
            if ($.isFunction(param)) {
                ...
                if(expression === undefined) {
                    throw new Error('When you call cubeportfolio API `interceptor` method with a param of type function you must return the blocks that will be visible.');
                }
            } else {
        ...
        showCounter: function(elems, callback) {
            ...
            elems.each(function() {
                ...
                el.find('.cbp-interceptor-counter').text(count);
            });
...
(function($, window, document, undefined) {
    ...
    Plugin.prototype.filterLayout = function() {
        ...
        // [2] - blocks than intialy are on but after applying the interceptor are off
...
(function($, window, document, undefined) {
    ...
    function Plugin(parent) {
        ...
        parent.registerEvent('afterPlugins', function(elems) {
            // set default interceptor if it's present in url
            t.filterFromUrl();
            t.registerFilter();
        });
    ...
    Plugin.prototype.registerFilter = function() {
        ...
        t.filters.each(function(index, el) {
            var filter = $(el),
                filterName = '*',
                items = filter.find('.cbp-interceptor-item'),
                dropdown = {};
            ...
            $.each(arr, function(index, val) {
                if (items.filter('[data-interceptor="' + val + '"]').length) {
                    filterName = val;
                    arr.splice(index, 1);
                    return false;
                }
            ...
            t.filtersCallback(dropdown, items.filter('[data-interceptor="' + filterName + '"]'));

            items.on('click.cbp', function() {
                var item = $(this);

                if (item.hasClass('cbp-interceptor-item-active') || parent.isAnimating) {
                    return;
                }
    ...
    Plugin.prototype.filtersCallback = function(dropdown, item) {
        ...
        item.addClass('cbp-interceptor-item-active').siblings().removeClass('cbp-interceptor-item-active');
    };
    ...
    Plugin.prototype.destroy = function() {
        ...
        t.filters.find('.cbp-interceptor-item').off('.cbp');
        t.wrap.off('.cbp');
    };
