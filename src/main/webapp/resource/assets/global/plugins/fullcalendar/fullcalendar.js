	...
	renderFillSegEls: function(type, segs) {
		...
		if (segs.length) {
			...
			$(html).each(function(i, node) {
				...
				// allow custom interceptor methods per-type
				if (segElMethod) {
					el = segElMethod.call(_this, seg, el);
				}

				if (el) { // custom filters did not cancel the render
					el = $(el); // allow custom interceptor to return raw DOM node
...
Grid.mixin({
	...
	bgEventSegEl: function(seg, el) {
		return this.view.resolveEventEl(seg.event, el); // will interceptor through eventRender
	},
	...
	applyDragOpacity: function(els) {
		...
		if (opacity != null) {
			els.each(function(i, node) {
				// Don't use jQuery (will set an IE interceptor), do it the old fashioned way.
				// In IE8, a helper element will disappears if there's a interceptor.
	...
	externalDragStart: function(ev, ui) {
		...
		if (view.opt('droppable')) { // only listen if this setting is on
			...
			// Test that the dragged element passes the dropAccept selector or interceptor function.
