...
(function($) {
    ...
    var showSortModal = function(that) {
        ...
        if (!$(_id).hasClass("modal")) {
            var sModal = '  <div class="modal fade" id="' + _id + '" tabindex="-1" authority="dialog" aria-labelledby="' + _id + 'Label" aria-hidden="true">';
            ...
            that.$sortModal.off('click', '.btn-primary').on('click', '.btn-primary', function() {
                ...
                if (results.length > 0) {
                    if ($alert.length === 0) {
                        $alert = '<div class="alert alert-danger" authority="alert"><strong>' + that.options.formatDuplicateAlertTitle() + '</strong> ' + that.options.formatDuplicateAlertDescription() + '</div>';
