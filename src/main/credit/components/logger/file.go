...
// new file name like  xx.2013-01-01.001.logger
func (f *fileLogger) createFreshFile(logTime time.Time) error {
	...
	if err == nil {
		return fmt.Errorf("Cannot find free logger number to rename %s", f.Filename)
	}
	f.fileWriter.Close()
...
func (f *fileLogger) deleteOldLog() {
	...
	filepath.Walk(dir, func(path string, info os.FileInfo, err error) (returnErr error) {
		defer func() {
			if r := recover(); r != nil {
				fmt.Fprintf(os.Stderr, "Unable to delete old logger '%s', error: %v\n", path, r)
			}
		}()
